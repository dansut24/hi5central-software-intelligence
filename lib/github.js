...
export async function searchGithubRepositories(query, limit = 10) {
  ...
  for (const repo of json.items || []) {
    const packageId = repo.full_name;

    try {
      const details = await getGithubReleaseDetails(packageId);
      ...
      results.push({
        ...
        silent_install_args: details.silent_install_args,
        silent_uninstall_args: details.silent_uninstall_args,
        importable: true,
      });
    } catch (error) {
      results.push({
